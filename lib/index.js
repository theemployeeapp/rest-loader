...
module.exports = function RESTLoader(options) {
  ...
  var makeUpdateRequest = function makeUpdateRequest(data) {
    if(!_.isUndefined(data[identifier])) {
      var requestOpts = {
        uri: data[identifier].toString(),
        method: 'PUT',
        json: data
      };
      return makeRequest(requestOpts);
    } else {
      throw new Error('identifier field on data is undefined');
    }
  };
